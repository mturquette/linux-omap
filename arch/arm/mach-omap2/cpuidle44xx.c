			...
			struct cpuidle_state *state)
{
	...
	if (cx->core_state < PWRDM_POWER_ON) {
		/* Clear OMAP44XX_IRQ_SYS_2N  */
		omap4_wakeupgen_clear_interrupt(dev->cpu, OMAP44XX_IRQ_SYS_2N);
	...
	if (cx->core_state < PWRDM_POWER_ON) {
		...
		/* set OMAP44XX_IRQ_SYS_2N  */
		omap4_wakeupgen_set_interrupt(dev->cpu, OMAP44XX_IRQ_SYS_2N);
	}
