...
static struct dpll_cascade_saved_state {
	...
	unsigned long vdd_iva_nom_volt;
	unsigned long vdd_core_nom_volt;
} state;
...
int omap4_dpll_low_power_cascade_enter()
{
	...
	struct omap_opp *opp;
	struct voltagedomain *vdd_iva, *vdd_core;
	...
	/* scale VDD_CORE to OPP50 voltage */
	state.vdd_core_nom_volt = omap_voltage_get_nom_volt(vdd_core);
	ret = omap_voltage_scale(vdd_core, 930000);
	if (ret)
		pr_err("%s: failed to scale vdd_core to OPP50 voltage\n",
				__func__);
	...
	/* scale VDD_IVA voltage */
	state.vdd_iva_nom_volt = omap_voltage_get_nom_volt(vdd_iva);
	ret = omap_voltage_scale(vdd_iva, 930000);
	if (ret)
		pr_err("%s: failed to scale VDD_IVA to OPP50 voltage\n",
				__func__);
...
int omap4_dpll_low_power_cascade_exit()
{
	...
	if (!dpll_abe_ck || !dpll_mpu_ck || !div_mpu_hs_clk || !dpll_iva_ck ||
		...
		|| !per_hs_clk_div_ck || !vdd_iva || !vdd_core) {
		pr_warn("%s: failed to get all necessary clocks\n", __func__);
		ret = -ENODEV;
		goto out;
	}
	...
	/* restore vdd_iva voltage */
	omap_voltage_scale(vdd_iva, state.vdd_iva_nom_volt);
	...
	/* restore vdd_core voltage */
	omap_voltage_scale(vdd_core, state.vdd_core_nom_volt);
